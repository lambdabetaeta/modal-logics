...
dual-axK = DGL-lam (DGL-lam (
  DGL-boxE (DGL-var (pop top))
          (DGL-boxE (DGL-var top)
                   (DGL-boxI (DGL-app (DGL-var (pop top)) (DGL-var top))))))

dual-ax4 : ∀ {Δ Γ A} -> Δ / Γ ⊢ □ A => □ □ A
dual-ax4 = DGL-lam (DGL-boxE (DGL-var top) (DGL-boxI (DGL-boxI (DGL-var top))))
...
GL-hilb-to-dual (GL-MP d d₁) = DGL-app (GL-hilb-to-dual d) (GL-hilb-to-dual d₁)
GL-hilb-to-dual (GL-NEC d) = DGL-boxI (GL-hilb-to-dual d)
...
GL-hilb-to-dual GL-axW = DGL-boxY
...
GL-dual-to-hilb (DGL-app d d₁) = GL-MP (GL-dual-to-hilb d) (GL-dual-to-hilb d₁)
...
GL-dual-to-hilb {Δ} {Γ} {□ A} (DGL-boxI d) =
  GL-weak (weakmany (boxcx Δ) Γ)
          (GL-normal4-ded (GL-dual-to-hilb d))
...
GL-dual-to-hilb (DGL-boxY) = GL-axW
