...
ne-weak (STLC-ne-app p x) q = STLC-ne-app (ne-weak p q) (weak x q)
...
sem (STLC-app p q) E = sem p E subsetid (sem q E)
...
reify {A = P i}  e = STLC-nf-ne e
reify {A = A => B} e =
  STLC-nf-lam (reify (e (weakone subsetid) (reflect (STLC-ne-var top))))
reify {A = A ∧ B} e =
  STLC-nf-prod (reify (proj₁ e)) (reify (proj₂ e))

reflect {A = P x} m = m
reflect {A = A => B} m p x =
  reflect (STLC-ne-app (ne-weak m p) (insert-nf (reify x)))
reflect {A = A ∧ B} p =
  pair (reflect (STLC-ne-fst p)) (reflect (STLC-ne-snd p))
...
term2 :  ∀ {A B} -> · , (A => B) , A ⊢ B
term2 = STLC-app (STLC-app (weak term subsetempty) (STLC-var (pop top))) (STLC-var top)
